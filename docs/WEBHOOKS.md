...
Every request carries an `x-webhook-signature` header — an HMAC-SHA256 of
`<timestamp>.<raw_body>` computed with your subscription secret — and an
`x-webhook-timestamp` header (ms epoch). Bind the signature to the timestamp and
reject anything older than five minutes to defeat replay.
...
To keep signing keys fresh, rotate your webhook secret periodically. SwiftRemit
performs a **zero-downtime rotation with a 24-hour overlap window**.

When a rotation is triggered SwiftRemit generates a new secret immediately, but
for the next 24 hours it signs every delivery **twice** so in-flight deliveries
and your own deploy don't drop events:

- `x-webhook-signature` — signed with the **new** secret.
- `x-webhook-signature-prev` — signed with the **previous** secret.

After the overlap window the previous secret is retired and
`x-webhook-signature-prev` is no longer sent.

Your verification logic should check the current signature first and, only if
that fails, fall back to the previous signature. Keep both secrets in your
configuration for the duration of the rotation so you can deploy the new secret
...
function verifyWithRotation(
  ...
  const ts = headers['x-webhook-timestamp'];
  if (!ts || Math.abs(Date.now() - Number(ts)) > 300_000) return false;
  const msg = `${ts}.${rawBody}`;

  // 1. Try the current secret.
  if (isValidSignature(msg, headers['x-webhook-signature'], currentSecret)) {
    return true;
  }

  // 2. Fall back to the previous secret during the rotation overlap.
  const prevSig = headers['x-webhook-signature-prev'];
...
function isValidSignature(msg: string, signature: string | undefined, secret: string): boolean {
