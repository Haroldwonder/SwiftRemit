# Issue 4: The real money-movement endpoint bypassed the KYC transfer guard
...
never imported or called `createTransferGuard`/`transferGuard` at all. That
...
- `POST /api/remittance` is now defined as
  ...
  before the handler is allowed to reach the `INSERT INTO transactions`
  call.
- The `/api/transfer` stub in `routes/kyc.ts` was left as-is (per the
  issue's guidance, the priority was closing the gap on the endpoint that
  actually moves money; the stub is a separate, lower-risk cleanup item).
...
code for a caller whose KYC isn't in good standing, instead of unconditionally
creating a pending transaction for any request carrying an `x-user-id`
header.
