...
import {
  ...
  initApiKeyMiddleware,
} from './middleware/api-key-rate-limit';
import { ApiKeyStore, ALL_SCOPES, ApiKeyScope, RateLimitTier } from './middleware/api-key-store';
...
// Initialise API key store (creates table if needed, wires per-tier limiters)
...
// Scope + per-tier rate limit enforcement for all /api/ routes
app.use('/api/', scopedApiKeyMiddleware);
